...
bool GraphSignal::HasYPos(const unsigned int yPos)const
{
    TimingDocument *doc = (TimingDocument *)m_view->GetDocument();

    unsigned int prespace = m_sig->prespace;
    return yPos > m_yoffset + prespace &&
        yPos < m_yoffset + prespace + doc->SignalHeight;
}
